...
int main(int argc, char *argv[]){
    ...
    if ((pid = fork()) == 0) {
        ...
        sleep(5);                                           // Sleep so parent will map shared memory

        char* child_shmem = (char*)child_size_before_mapping;
        ...
        map_shared_pages(getpid(), pid, shmem, PGSIZE); // AShare allocated memory

        sleep(10);                                      // Sleep so child will write to shared memory
